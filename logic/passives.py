...
class PassiveWagTail(BasePassive):
    ...
    description = "Один раз за раунд: При безответной атаке выставляет Уклонение (5-7). Восстанавливается в начале следующего хода."

    def get_virtual_defense_die(self, unit, incoming_die):
        # 1. Проверяем, использовали ли мы способность в этом раунде
        # Используем память юнита, чтобы хранить состояние
        mem_key = f"{self.id}_used"
        if unit.memory.get(mem_key, False):
            return None  # Уже виляли хвостиком, устали :(

        # 2. Если не использовали — выдаем кубик и помечаем как использованное
        unit.memory[mem_key] = True
    ...
    def on_round_end(self, unit, log_func):
        # Сбрасываем счетчик в конце раунда
        mem_key = f"{self.id}_used"
        if unit.memory.get(mem_key):
            unit.memory[mem_key] = False
            # Можно добавить лог, если хочется
            # if log_func: log_func(f"🔄 {self.name}: Готов уклоняться снова!")
